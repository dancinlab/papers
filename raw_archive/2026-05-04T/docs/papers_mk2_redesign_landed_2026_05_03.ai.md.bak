...
- **C1** — 기존 single-file SSOT (`manifest.json` 183KB / 433 paper entry) 가 작동 중. mk2 raw 270/271/272/273 triplet (core/<도메인>/{source,registry,router,main} + modules/<도메인>/) 으로 재정합 시 대규모 file 구조 변경 필수 — 본 BG 는 additive only 정책으로 마이그레이션 보류. `.roadmap.*` 4개만 신규 land.
...
- **C5** — `bin/papers` 12-verb 와 mk2 raw 270/271/272/273 triplet 의 정합 deferred. 현재 single bash entry + tool/* delegation 구조 유지 — raw 270 router pattern 도입 시 bin/papers refactor 필요 (Phase 3 candidate).
