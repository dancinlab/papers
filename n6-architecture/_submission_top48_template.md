...
본 파일은 상위 48편을 저널/컨퍼런스 제출 포맷으로 정리하기 위한 표준 템플릿이다. 실제 논문 본문은 기존 `papers/n6-*-paper.md` 그대로 두고, 제출 시에는 이 템플릿의 헤더/초록/참고문헌 블록만 덧씌운다. WHY / COMPARE / MAIN / VERIFICATION 4단 구조는 CANON 모든 논문의 공통 골격이다.
...
```
저자: 박민우 (CANON 주관), NEXUS-6 AI 협업체
소속: CANON 프로젝트 (AI-native Arithmetic Design Framework)
연락처: 저장소 이슈 트래커 경유
...
```
[1] CANON 내부 참조: atlas.n6 노드 경로 + 인용 줄.
[2] BT-NNN (BreakThrough 레지스트리 `_registry.json` section id).
[3] 외부 문헌은 도메인별로 3~10 편 제시. CrossRef DOI 존재할 것.
[4] CANON DOI 간 상호 인용은 시뮬 DOI `10.NEXUS6.n6-arch/2026-NNN` 로 명시하되 "시뮬" 표기를 유지.
```
...
- 실제 저널 투고는 별도 프로세스. 본 템플릿은 CANON 내부 아카이브 표준이다.
